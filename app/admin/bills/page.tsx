      ...
      <div className="max-w-6xl mx-auto">
        ...
        {resident && (
          <div className="mt-4 p-5 rounded-3xl bg-white border">
            ...
            <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
              ...
              <div className="p-3 rounded-2xl bg-slate-900 text-white"><div className="opacity-60">Total Due</div><div className="font-bold text-sm">₹{totalDue}</div></div>
        ...
        <div className="mt-6 p-4 rounded-3xl bg-white border">
                ...
                <tbody>
                  {bills.map(b=>(
                    <tr key={b.id} className="border-b hover:bg-slate-50">
                      ...
                      <td className="p-3">{ledger.find(l=>l.bill_id===b.id)? <span className="text- bg-black text-white px-2 py-1 rounded-full">Credit OK</span> : <span className="text- opacity-40">-</span>}</td>
                    </tr>
                  ))}
          ...
          <div className="mt-4 grid grid-cols-3 gap-2 text-xs font-bold">
            <div className="p-3 rounded-2xl bg-slate-900 text-white">Total Due in Filter: ₹{pending.reduce((s,b)=>s+Number(b.amount),0)}</div>
            <div className="p-3 rounded-2xl bg-emerald-600 text-white">Total Paid in Filter: ₹{paid.reduce((s,b)=>s+Number(b.amount),0)}</div>
