...
export default function GuardMobile() {
  ...
  const handleDelivery = async () => {
    ...
    if(res) alert(`📦 Delivery Approval Sent to ${resident.name}`)
  }
      ...
      <div className="flex-1 max-w-md w-full mx-auto px-4 py-4 pb-24">
          ...
          {flat && (
            <div className={`mt-3 p-3 rounded-2xl border flex gap-2.5 items-center ${resident?'bg-emerald-50 border-emerald-100':'bg-red-50 border-red-100'}`}>
              ...
              <div className="text-xs flex-1">
                {resident? (<><div className="font-bold">{flat.toUpperCase()} • {resident.name} • {resident.mobile}</div><div className="text-emerald-700 font-bold">✅ Resident Verified</div></>):(<><div className="font-bold text-red-700">Resident Not Found</div><div className="text-slate-500">Supabase residents me {flat.toUpperCase()} add karo</div></>)}
              </div>
            </div>
          )}
        ...
        {tab==='approved' && (
          <div className="mt-4 p-5 rounded-3xl bg-white border shadow-sm">
            <div className="text-sm font-bold">Inside + Pending</div>
            <div className="mt-3 space-y-2 max-h- overflow-auto">
              ...
              {insideList.map(v=>(
                <div key={v.id} className="p-3 rounded-2xl bg-emerald-50 border border-emerald-100 flex justify-between items-center"><div><div className="font-bold text-sm">✅ {v.visitor_name||v.name} → {v.flat_no}</div></div><button onClick={()=>markExit(v.id)} className="px-3 py-1.5 rounded-full bg-slate-900 text-white text-xs font-bold">Exit</button></div>
              ))}
            </div>
          </div>
        )}
        ...
        {tab==='exit' && (
          <div className="mt-4 p-5 rounded-3xl bg-white border shadow-sm">
            <input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder="Name / Flat search karo" className="w-full h-12 rounded-2xl bg-slate-50 border px-4 text-sm"/>
            <div className="mt-3 space-y-2">
              {allVisitors.filter(v=>v.status==='inside' && (v.visitor_name?.toLowerCase().includes(searchQuery.toLowerCase()) || v.flat_no?.toLowerCase().includes(searchQuery.toLowerCase()) ||!searchQuery)).slice(0,20).map(v=>(
                <div key={v.id} className="p-3 rounded-2xl bg-white border shadow-sm flex justify-between items-center"><div><div className="font-bold text-sm">{v.visitor_name||v.name} → {v.flat_no}</div><div className="text- text-slate-500">Inside</div></div><button onClick={()=>markExit(v.id)} className="px-4 py-2 rounded-full bg-slate-900 text-white text-xs font-bold">Exit Karo</button></div>
              ))}
            </div>
          </div>
        )}
        ...
        {tab==='emergency' && (
          <div className="mt-4 p-5 rounded-3xl bg-white border shadow-sm">
            {!resident && <div className="p-3 rounded-2xl bg-red-50 text-xs text-red-700">Flat No daalo</div>}
            {resident && (
              <div className="space-y-3">
                <select value={emergencyType} onChange={e=>setEmergencyType(e.target.value)} className="w-full h-12 rounded-2xl bg-red-50 border border-red-100 px-3 text-sm font-bold"><option>Fire</option><option>Medical</option><option>Theft</option></select>
                <textarea value={emergencyNote} onChange={e=>setEmergencyNote(e.target.value)} placeholder="Detail..." className="w-full h-24 rounded-2xl bg-slate-50 border p-3 text-sm"/>
                <button onClick={handleEmergency} disabled={loading} className="w-full h-12 rounded-full bg-red-600 text-white font-bold text-sm">🚨 Emergency Bhejo</button>
              </div>
            )}
          </div>
        )}
