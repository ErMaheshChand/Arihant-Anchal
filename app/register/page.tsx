...
export default function RegisterPage(){
  ...
  useEffect(()=>{
    const load = async ()=>{
      const all: string[] = []
      for(let b of ['A','B','C','D']) for(let i=101;i<=110;i++) all.push(`${b}-${i}`)
      for(let i=301;i<=310;i++) all.push(`B-${i}`)
      const { data } = await supabase.from('residents').select('flat_no').eq('status','approved')
      const setApproved = new Set((data||[]).map((r:any)=>r.flat_no))
      setFlats(all.filter(f=>!setApproved.has(f)))
    }
    load()
  },[])
      ...
      <div className="flex-1 overflow-auto p-4">
        <div className="max-w-md mx-auto bg-white text-black rounded- p-5 space-y-4 shadow-2xl">
          <div className="flex gap-4 items-center">
            <div className="w-20 h-20 rounded- bg-slate-100 border-2 border-dashed flex items-center justify-center overflow-hidden">{preview?<img src={preview} className="w-full h-full object-cover"/>:'👤'}</div>
            <label className="px-4 h-8 rounded-full bg-slate-900 text-white text-xs font-bold flex items-center cursor-pointer">📷 Upload<input type="file" accept="image/*" className="hidden" onChange={e=>setPhoto(e.target.files?.[0]||null)}/></label>
          </div>
          <select value={form.flat_no} onChange={e=>setForm({...form, flat_no:e.target.value})} className="w-full h-12 rounded-2xl bg-slate-50 border px-4 font-bold text-sm"><option value="">Flat Select</option>{flats.map(f=><option key={f} value={f}>{f}</option>)}</select>
          ...
          <button onClick={register} disabled={loading} className="w-full h-12 rounded-full bg-amber-400 text-black font-bold text-sm">{loading?'Sending...':'📩 Admin Approval ke liye Bhejo'}</button>
        </div>
      </div>
    </div>
  )
