...
export default function ResidentApp() {
  ...
  const SOCIETY_UPI = 'anchalsociety@okicici' // <-- apna UPI yaha dalo
  ...
  const downloadReceipt = (bill:any)=>{
    const doc = new jsPDF()
    doc.setFontSize(18)
    doc.text('Anchal Society - Payment Receipt', 20, 20)
    doc.setFontSize(11)
    ...
    doc.setFontSize(10)
    doc.text(`Thank you for payment!`, 20, 90)
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-black flex flex-col" onClick={()=>{ if(!soundEnabled) enableSound() }}>
      ...
      <div className="sticky top-0 z-40 bg-white/90 backdrop-blur rounded-b-3xl border-b px-4 h-14 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-2xl bg-slate-900 text-amber-300 flex items-center justify-center font-bold">A</div><div className="leading-tight"><div className="font-bold text-sm">{flatNo} • Aarav Sharma</div><div className="text- text-slate-500">Owner</div></div></div>
      </div>

      <div className="flex-1 max-w-md w-full mx-auto px-4 py-4 pb-24">
        {tab==='home' && (<div className="rounded-3xl bg-slate-900 text-white p-5"><div className="text- opacity-60 font-bold">TOTAL DUE</div><div className="text-2xl font-bold">₹{totalDue}</div><button onClick={()=>setTab('bills')} className="mt-3 w-full h-10 rounded-full bg-white text-black text-xs font-bold">View & Pay →</button></div>)}

        {tab==='visitors' && (
          <div>
            <h2 className="text-lg font-bold">Pre-Approve</h2>
            <div className="mt-3 p-5 rounded-3xl bg-white border space-y-3">
            ...
            <div className="mt-6"><div className="flex justify-between"><h3 className="text-sm font-bold">Visitor History</h3></div>
              <div className="mt-3 p-3 rounded-2xl bg-white border flex gap-2">
                <div className="flex-1"><div className="text- font-bold">From</div><input type="date" value={fromDate} onChange={e=>setFromDate(e.target.value)} className="w-full h-9 rounded-xl bg-slate-50 border px-2 text-xs" /></div>
                <div className="flex-1"><div className="text- font-bold">To</div><input type="date" value={toDate} onChange={e=>setToDate(e.target.value)} className="w-full h-9 rounded-xl bg-slate-50 border px-2 text-xs" /></div>
              ...
              <div className="mt-3 space-y-2">{allVisitors.map((v:any)=>(<div key={v.id} className="p-3 rounded-2xl bg-white border flex justify-between"><div className="text-sm font-bold">{v.name || v.visitor_name}</div><div className="text- font-black">{v.status}</div></div>))}</div>
            </div>
          </div>
        )}
        ...
        {tab==='bills' && (
          <div>
            ...
            <div className="mt-3 rounded-3xl bg-slate-900 text-white p-5"><div className="text- opacity-60 font-bold">TOTAL PENDING DUE</div><div className="text-3xl font-bold">₹{totalDue}</div><div className="mt-3 grid grid-cols-3 gap-2 text-xs"><div className="p-2.5 rounded-2xl bg-white/10"><div className="opacity-60">Monthly</div><div className="font-bold">₹{monthlyDue}</div></div><div className="p-2.5 rounded-2xl bg-white/10"><div className="opacity-60">Old Due</div><div className="font-bold">₹{oldDue}</div></div><div className="p-2.5 rounded-2xl bg-white/10"><div className="opacity-60">Other</div><div className="font-bold">₹{otherDue}</div></div></div></div>

            <div className="mt-5 p-3 rounded-2xl bg-white border flex gap-2">
              <div className="flex-1"><div className="text- font-bold">From Date</div><input type="date" value={billFrom} onChange={e=>setBillFrom(e.target.value)} className="w-full h-9 rounded-xl bg-slate-50 border px-2 text-xs font-bold" /></div>
              <div className="flex-1"><div className="text- font-bold">To Date</div><input type="date" value={billTo} onChange={e=>setBillTo(e.target.value)} className="w-full h-9 rounded-xl bg-slate-50 border px-2 text-xs font-bold" /></div>
            ...
            <div className="mt-6"><div className="text-sm font-bold">Payment History (with Receipt)</div><div className="mt-3 rounded-3xl bg-white border overflow-hidden"><div className="max-h- overflow-y-auto"><table className="w-full text-xs"><thead className="sticky top-0 bg-slate-50 border-b text- font-bold"><tr><th className="text-left p-3">Date/Time</th><th className="text-left p-3">Title</th><th className="text-right p-3">Amt</th><th className="text-right p-3">Action</th></tr></thead><tbody>{bills.map(b=>(<tr key={b.id} className="border-b"><td className="p-3"><div className="font-bold">{b.paid_at? new Date(b.paid_at).toLocaleDateString() : b.due_date}</div><div className="text- text-slate-400">{b.paid_at? new Date(b.paid_at).toLocaleTimeString() : 'Pending'}</div><div className="text-">{b.receipt_no||''}</div></td><td className="p-3">{b.title}<div className={`text- px-1 py-0.5 rounded-full inline-block ${b.status==='paid'?'bg-emerald-50 text-emerald-700':'bg-amber-50 text-amber-700'}`}>{b.status}</div></td><td className="p-3 text-right font-bold">₹{b.amount}</td><td className="p-3 text-right">{b.status==='paid'? <button onClick={()=>downloadReceipt(b)} className="px-3 py-1 rounded-full bg-black text-white text-">Receipt PDF</button> : '-'}</td></tr>))}</tbody></table></div></div></div>
          </div>
        )}
