...
export default function ResidentApp() {
  ...
  useEffect(()=>{
    if(!flatNo) return
    loadVisitors(); loadBills(); loadBookings()
    ...
    return ()=>{ supabase.removeChannel(ch); supabase.removeChannel(ch2); supabase.removeChannel(ch3); supabase.removeChannel(ch4); supabase.removeChannel(ch5); supabase.removeChannel(ch6); if(beepInterval.current) clearInterval(beepInterval.current) }
  },[flatNo])
      ...
      <div className="flex-1 max-w-md w-full mx-auto px-4 py-4 pb-24">
        ...
        {tab==='bills' && (
          <div>
            ...
            <div className="mt-3 rounded-3xl bg-slate-900 text-white p-5 relative overflow-hidden">
              ...
              <div className="mt-1 text-xs bg-white/10 rounded-full px-2 py-1 inline-block">Paid Total (Ledger): ₹{totalCollection} | Cash: ₹{totalCash} | Online: ₹{totalCollection-totalCash}</div>
            ...
            <div className="mt-5 p-3 rounded-2xl bg-white border flex gap-2">
              <div className="flex-1"><div className="text-xs font-bold text-slate-500">From Date</div><input type="date" value={billFrom} onChange={e=>setBillFrom(e.target.value)} className="w-full h-9 rounded-xl bg-slate-50 border px-2 text-xs font-bold" /></div>
              <div className="flex-1"><div className="text-xs font-bold text-slate-500">To Date</div><input type="date" value={billTo} onChange={e=>setBillTo(e.target.value)} className="w-full h-9 rounded-xl bg-slate-50 border px-2 text-xs font-bold" /></div>
            ...
            <div className="mt-6">
              <div className="flex justify-between items-center">
                ...
                <div className="text-xs bg-black text-white px-2 py-1 rounded-full">{combinedHistory.length} receipts</div>
              </div>
              <div className="mt-3 rounded-3xl bg-white border overflow-hidden shadow-sm">
                <div className="max-h- overflow-y-auto overflow-x-hidden" style={{scrollbarWidth:'thin'}}>
                  <table className="w-full text-xs">
                    <thead className="sticky top-0 z-10 bg-slate-900 text-white text-xs font-bold">
                    ...
                    <tbody>
                      {combinedHistory.map((h:any)=>(
                        <tr key={h.id} className="border-b last:border-0 hover:bg-slate-50">
                          <td className="p-3"><div className="font-bold">{new Date(h.created_at).toLocaleDateString()}</div><div className="text-xs text-slate-400">{new Date(h.created_at).toLocaleTimeString()}</div><div className="text-xs opacity-60 truncate max-w-">{h.receipt_no||''}</div></td>
                          <td className="p-3"><div className="font-medium truncate max-w-">{h.title}</div><div className="flex gap-1 mt-1"><span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${h.pay_mode==='CASH'?'bg-amber-100 text-amber-700':'bg-blue-100 text-blue-700'}`}>{h.pay_mode}</span><span className="text-xs px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 font-bold">PAID</span></div></td>
                          <td className="p-3 text-right font-bold">₹{h.amount}</td>
                          <td className="p-3 text-right"><button onClick={()=>downloadReceipt(h)} className="px-3 py-1 rounded-full bg-black text-white text-xs">PDF</button></td>
                        </tr>
                      ))}
                ...
                <div className="p-2 bg-slate-50 text-xs text-center font-bold text-slate-500 border-t">
        ...
        {tab==='book' && (
          <div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              {facilities.map((f,i)=>(
                <button key={i} onClick={()=>setBookForm({...bookForm, facility:f.name, price:f.price})}
                  ...
                  <div className="text- opacity-60">₹{f.price}/hr</div>
                </button>
              ))}
            </div>
            <div className="mt-4 p-4 rounded-3xl bg-white border space-y-3">
              ...
              <div className="p-3 rounded-2xl bg-blue-50 border border-blue-200 text-xs font-bold">
                Selected: {bookForm.facility||'-'} | {bookForm.date} {bookForm.start}-{bookForm.end} | Est. ₹{(()=>{ const s=parseInt(bookForm.start.split(':')[0]||'0'); const e=parseInt(bookForm.end.split(':')[0]||'0'); const h=Math.max(1,e-s); return h*(bookForm.price||0) })()}
              </div>
            ...
            <div className="mt-5">
              <div className="text-sm font-bold flex justify-between"><span>My Bookings</span><span className="text- bg-black text-white px-2 py-1 rounded-full">{myBookings.length}</span></div>
              <div className="mt-2 space-y-2 max-h- overflow-y-auto">
                {myBookings.map((b:any)=>(
                  <div key={b.id} className="p-3 rounded-2xl bg-white border flex justify-between items-center">
                    <div><div className="font-bold text-sm">{b.facility_name}</div><div className="text- opacity-60">{b.booking_date} {b.start_time}-{b.end_time} • {b.hours}hr • ₹{b.amount}</div></div>
                    <span className={`text- px-2 py-1 rounded-full font-bold ${b.status==='approved'?'bg-emerald-50 text-emerald-700 border border-emerald-200':b.status==='pending'?'bg-amber-50 text-amber-700 border border-amber-200':'bg-red-50 text-red-700 border'}`}>{b.status.toUpperCase()}</span>
                  </div>
                ))}
                {myBookings.length===0 && <div className="p-6 text-center text-xs opacity-40 bg-white rounded-2xl border">No bookings yet - upar se book karo</div>}
              </div>
              <div className="mt-1 text- text-center font-bold">↕️ Scroll karo - saari bookings</div>
            </div>
          </div>
        )}
